...
def deskew_pdf(input_pdf, output_pdf):
    """Extract images, deskew them with high quality, and save to a new PDF."""
    ...
    for page in doc:
        ...
        for img in img_list:
            ...
            pil_corrected.save(img_bytes, format=img_ext.upper(), quality=100)  # Preserve max quality

            new_page.insert_image(new_page.rect, stream=img_bytes.getvalue())

    new_doc.save(output_pdf, deflate=False)  # No compression
    new_doc.close()
    doc.close()
